...
const Selected_Geeks: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const res = await axios.get<Application[]>(`https://jobera.onrender.com/api/get-application?username=${currentUser?.username}&status=Selected`);
        if (res.status === 200) {
