...
const Rejected_Geeks: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const res = await axios.get<Application[]>(`https://jobera.onrender.com/api/get-application?username=${currentUser?.username}&status=Rejected`);
        if (res.status === 200) {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8">
      <h1 className="text-2xl font-bold mb-4 text-center">Rejected Applications</h1>
      {isLoading ? (
            ...
            <tbody className="divide-y divide-gray-200">
              {applications.map((app) => (
                <tr key={app.id}>
                  ...
                  <td className={`py-4 px-6 ${getStatusColor(app.status)}`}>{app.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
